  ...
  class BlogAuthorDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::BlogAuthorDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::BlogAuthorDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, timestamp, qualified_name, birthday, first_name, last_name, public_name, language_id, timezone_id, country_id, city_id, state_id, about, job_title, cover_url, avatar_url, git_hub_url, website_url, twitter_url, facebook_url, you_tube_url, linked_in_url, instagram_url, social_feed_id, social_profile_id, gender, wallet_id, status, user_name, public_identifier, phone_number_confirmed, availability].hash
    end
