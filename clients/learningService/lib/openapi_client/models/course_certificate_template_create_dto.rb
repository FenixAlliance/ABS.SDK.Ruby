  ...
  class CourseCertificateTemplateCreateDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CourseCertificateTemplateCreateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CourseCertificateTemplateCreateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @course_id.to_s.length > 36
        invalid_properties.push('invalid value for "course_id", the character length must be smaller than or equal to 36.')
      end
    ...
    def course_id=(course_id)
      ...
      if course_id.to_s.length > 36
        fail ArgumentError, 'invalid value for "course_id", the character length must be smaller than or equal to 36.'
      end

      if course_id.to_s.length < 36
        fail ArgumentError, 'invalid value for "course_id", the character length must be great than or equal to 36.'
      end
