...
> <EmptyEnvelope> patch_course_article_async(tenant_id, article_id, opts)
...
begin
  # Patch a course article
  result = api_instance.patch_course_article_async(tenant_id, article_id, opts)
  p result
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseArticlesApi->patch_course_article_async: #{e}"
end
```

#### Using the patch_course_article_async_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<EmptyEnvelope>, Integer, Hash)> patch_course_article_async_with_http_info(tenant_id, article_id, opts)

```ruby
begin
  # Patch a course article
  data, status_code, headers = api_instance.patch_course_article_async_with_http_info(tenant_id, article_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <EmptyEnvelope>
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseArticlesApi->patch_course_article_async_with_http_info: #{e}"
end
```
