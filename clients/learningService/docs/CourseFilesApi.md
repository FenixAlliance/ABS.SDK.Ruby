...
> patch_course_file_async(tenant_id, file_id, opts)
...
begin
  # Patch a course file
  api_instance.patch_course_file_async(tenant_id, file_id, opts)
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseFilesApi->patch_course_file_async: #{e}"
end
```

#### Using the patch_course_file_async_with_http_info variant

This returns an Array which contains the response data (`nil` in this case), status code and headers.

> <Array(nil, Integer, Hash)> patch_course_file_async_with_http_info(tenant_id, file_id, opts)

```ruby
begin
  # Patch a course file
  data, status_code, headers = api_instance.patch_course_file_async_with_http_info(tenant_id, file_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseFilesApi->patch_course_file_async_with_http_info: #{e}"
end
```
