...
> patch_course_library_async(tenant_id, library_id, opts)
...
begin
  # Patch a course library
  api_instance.patch_course_library_async(tenant_id, library_id, opts)
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseLibrariesApi->patch_course_library_async: #{e}"
end
```

#### Using the patch_course_library_async_with_http_info variant

This returns an Array which contains the response data (`nil` in this case), status code and headers.

> <Array(nil, Integer, Hash)> patch_course_library_async_with_http_info(tenant_id, library_id, opts)

```ruby
begin
  # Patch a course library
  data, status_code, headers = api_instance.patch_course_library_async_with_http_info(tenant_id, library_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseLibrariesApi->patch_course_library_async_with_http_info: #{e}"
end
```
