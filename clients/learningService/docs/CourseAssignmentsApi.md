...
| [**patch_course_assignment_async**](CourseAssignmentsApi.md#patch_course_assignment_async) | **PATCH** /api/v2/LearningService/CourseAssignments/{assignmentId} | Patch a course assignment |
...
## patch_course_assignment_async

> <EmptyEnvelope> patch_course_assignment_async(tenant_id, assignment_id, opts)
...
begin
  # Patch a course assignment
  result = api_instance.patch_course_assignment_async(tenant_id, assignment_id, opts)
  p result
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseAssignmentsApi->patch_course_assignment_async: #{e}"
end
```

#### Using the patch_course_assignment_async_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<EmptyEnvelope>, Integer, Hash)> patch_course_assignment_async_with_http_info(tenant_id, assignment_id, opts)

```ruby
begin
  # Patch a course assignment
  data, status_code, headers = api_instance.patch_course_assignment_async_with_http_info(tenant_id, assignment_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <EmptyEnvelope>
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseAssignmentsApi->patch_course_assignment_async_with_http_info: #{e}"
end
```
