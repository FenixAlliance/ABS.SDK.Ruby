...
| [**patch_course_handout_async**](CourseHandoutsApi.md#patch_course_handout_async) | **PATCH** /api/v2/LearningService/CourseHandouts/{handoutId} | Patch a course handout |
...
## patch_course_handout_async

> patch_course_handout_async(tenant_id, handout_id, opts)
...
begin
  # Patch a course handout
  api_instance.patch_course_handout_async(tenant_id, handout_id, opts)
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseHandoutsApi->patch_course_handout_async: #{e}"
end
```

#### Using the patch_course_handout_async_with_http_info variant

This returns an Array which contains the response data (`nil` in this case), status code and headers.

> <Array(nil, Integer, Hash)> patch_course_handout_async_with_http_info(tenant_id, handout_id, opts)

```ruby
begin
  # Patch a course handout
  data, status_code, headers = api_instance.patch_course_handout_async_with_http_info(tenant_id, handout_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue OpenapiClient::ApiError => e
  puts "Error when calling CourseHandoutsApi->patch_course_handout_async_with_http_info: #{e}"
end
```
