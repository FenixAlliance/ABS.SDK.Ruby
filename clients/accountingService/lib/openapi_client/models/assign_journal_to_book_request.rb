  ...
  class AssignJournalToBookRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AssignJournalToBookRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AssignJournalToBookRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @financial_book_id.nil?
        invalid_properties.push('invalid value for "financial_book_id", financial_book_id cannot be nil.')
      end

      if @financial_book_id.to_s.length > 36
        invalid_properties.push('invalid value for "financial_book_id", the character length must be smaller than or equal to 36.')
      end

      if @financial_book_id.to_s.length < 0
    ...
    def valid?
      ...
      return false if @financial_book_id.nil?
      return false if @financial_book_id.to_s.length > 36
      return false if @financial_book_id.to_s.length < 0
    ...
    def financial_book_id=(financial_book_id)
      if financial_book_id.nil?
        fail ArgumentError, 'financial_book_id cannot be nil'
      end
