  ...
  class CreateLedgerDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CreateLedgerDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CreateLedgerDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ledger_type_id')
        self.ledger_type_id = attributes[:'ledger_type_id']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@ledger_type_id.nil? && @ledger_type_id.to_s.length > 36
        invalid_properties.push('invalid value for "ledger_type_id", the character length must be smaller than or equal to 36.')
      end

      if !@ledger_type_id.nil? && @ledger_type_id.to_s.length < 0
        invalid_properties.push('invalid value for "ledger_type_id", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@ledger_type_id.nil? && @ledger_type_id.to_s.length > 36
      return false if !@ledger_type_id.nil? && @ledger_type_id.to_s.length < 0
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] ledger_type_id Value to be assigned
    def ledger_type_id=(ledger_type_id)
      if !ledger_type_id.nil? && ledger_type_id.to_s.length > 36
        fail ArgumentError, 'invalid value for "ledger_type_id", the character length must be smaller than or equal to 36.'
      end

      if !ledger_type_id.nil? && ledger_type_id.to_s.length < 0
        fail ArgumentError, 'invalid value for "ledger_type_id", the character length must be great than or equal to 0.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ledger_type_id == o.ledger_type_id
    end
    ...
    def hash
      [id, timestamp, name, description, date_time, tenant_id, enrollment_id, ledger_type_id].hash
    end
