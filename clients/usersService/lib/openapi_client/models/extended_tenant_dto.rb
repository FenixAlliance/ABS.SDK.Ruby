  ...
  class ExtendedTenantDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ExtendedTenantDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ExtendedTenantDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, timestamp, qualified_name, kind, tax_id, about, wallet_id, social_feed_id, business_industry_id, business_segment_id, social_profile_id, language_id, name, duns, slogan, legal_name, cover_url, avatar_url, cart_id, currency_id, timezone_id, country_id, state_id, city_id, email, phone, web_url, facebook_url, twitter_url, git_hub_url, linked_in_url, instagram_url, you_tube_url, whats_app_number, support_phone_number, verified, business_name, business_legal_name, twitter_username, cart, wallet, social_profile].hash
    end
