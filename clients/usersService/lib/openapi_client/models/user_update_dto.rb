  ...
  class UserUpdateDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UserUpdateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UserUpdateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [qualified_name, birthday, first_name, last_name, public_name, id_provider, gender, email, about, status, job_title, timezone_id, language_id, currency_id, country_id, state_id, city_id, git_hub_url, website_url, twitter_url, facebook_url, you_tube_url, linked_in_url, instagram_url, web_url, availability].hash
    end
