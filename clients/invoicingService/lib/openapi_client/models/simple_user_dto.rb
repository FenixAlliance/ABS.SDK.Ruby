  ...
  class SimpleUserDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SimpleUserDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SimpleUserDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, timestamp, full_name, qualified_name, public_name, handler, last_name, first_name, cover_url, avatar_url, git_hub_url, country_id, timezone_id, website_url, twitter_url, you_tube_url, linked_in_url, facebook_url, instagram_url, social_profile_id].hash
    end
