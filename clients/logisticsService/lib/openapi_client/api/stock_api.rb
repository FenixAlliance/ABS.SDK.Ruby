  ...
  class StockApi
    ...
    def get_contacts_async_with_http_info(tenant_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: StockApi.get_contacts_async ...'
      end
      # verify the required parameter 'tenant_id' is set
      if @api_client.config.client_side_validation && tenant_id.nil?
        fail ArgumentError, "Missing the required parameter 'tenant_id' when calling StockApi.get_contacts_async"
      end
      # resource path
      local_var_path = '/api/v2/LogisticsService/Stock'
      ...
      new_options = opts.merge(
        :operation => :"StockApi.get_contacts_async",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: StockApi#get_contacts_async\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
