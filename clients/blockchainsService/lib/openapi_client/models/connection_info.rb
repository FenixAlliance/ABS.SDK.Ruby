  ...
  class ConnectionInfo
    ...
    def self.openapi_types
      {
        ...
        :'local_port' => :'Integer',
        :'client_certificate' => :'X509Certificate2'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ConnectionInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ConnectionInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'remote_ip_address')
      ...
      if attributes.key?(:'local_ip_address')
    ...
    def hash
      [id, remote_ip_address, remote_port, local_ip_address, local_port, client_certificate].hash
    end
