  ...
  class DealUnitFlowUpdateDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DealUnitFlowUpdateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DealUnitFlowUpdateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@parent_business_process_id.nil? && @parent_business_process_id.to_s.length > 36
        invalid_properties.push('invalid value for "parent_business_process_id", the character length must be smaller than or equal to 36.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] parent_business_process_id Value to be assigned
    def parent_business_process_id=(parent_business_process_id)
      if !parent_business_process_id.nil? && parent_business_process_id.to_s.length > 36
        fail ArgumentError, 'invalid value for "parent_business_process_id", the character length must be smaller than or equal to 36.'
      end

      if !parent_business_process_id.nil? && parent_business_process_id.to_s.length < 36
        fail ArgumentError, 'invalid value for "parent_business_process_id", the character length must be great than or equal to 36.'
      end
    ...
    def hash
      [name, description, parent_business_process_id].hash
    end
