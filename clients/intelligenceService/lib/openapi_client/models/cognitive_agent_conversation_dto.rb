  ...
  class CognitiveAgentConversationDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CognitiveAgentConversationDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CognitiveAgentConversationDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'activity_from')
        self.activity_from = attributes[:'activity_from']
      end
    ...
    def hash
      [id, timestamp, cognitive_agent_id, tenant_id, enrollment_id, title, claimed, current, channel_id, conversation_id, activity_id, activity_from, state, last_activity, account_holder_id, individual_id, organization_id, receiver_business_id].hash
    end
