...
module OpenapiClient
  class CognitiveAgentConversationUpdateDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CognitiveAgentConversationUpdateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CognitiveAgentConversationUpdateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@channel_id.nil? && @channel_id.to_s.length > 255
        invalid_properties.push('invalid value for "channel_id", the character length must be smaller than or equal to 255.')
      end

      if !@channel_id.nil? && @channel_id.to_s.length < 0
        invalid_properties.push('invalid value for "channel_id", the character length must be great than or equal to 0.')
      end

      if !@conversation_id.nil? && @conversation_id.to_s.length > 255
        invalid_properties.push('invalid value for "conversation_id", the character length must be smaller than or equal to 255.')
      end

      if !@conversation_id.nil? && @conversation_id.to_s.length < 0
        invalid_properties.push('invalid value for "conversation_id", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@channel_id.nil? && @channel_id.to_s.length < 0
      return false if !@conversation_id.nil? && @conversation_id.to_s.length > 255
      return false if !@conversation_id.nil? && @conversation_id.to_s.length < 0
    ...
    def channel_id=(channel_id)
      ...
      if !channel_id.nil? && channel_id.to_s.length < 0
        fail ArgumentError, 'invalid value for "channel_id", the character length must be great than or equal to 0.'
      end
    ...
    def conversation_id=(conversation_id)
      if !conversation_id.nil? && conversation_id.to_s.length > 255
        fail ArgumentError, 'invalid value for "conversation_id", the character length must be smaller than or equal to 255.'
      end

      if !conversation_id.nil? && conversation_id.to_s.length < 0
        fail ArgumentError, 'invalid value for "conversation_id", the character length must be great than or equal to 0.'
      end
    ...
    def hash
      [channel_id, conversation_id, state, current, claimed, account_holder_id, individual_id, organization_id, receiver_business_id].hash
    end
