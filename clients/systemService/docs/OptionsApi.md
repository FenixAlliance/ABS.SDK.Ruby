...
> <EmptyEnvelope> patch_system_option(option_id, opts)
...
begin
  # Partially update a system option
  result = api_instance.patch_system_option(option_id, opts)
  p result
rescue OpenapiClient::ApiError => e
  puts "Error when calling OptionsApi->patch_system_option: #{e}"
end
```

#### Using the patch_system_option_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<EmptyEnvelope>, Integer, Hash)> patch_system_option_with_http_info(option_id, opts)

```ruby
begin
  # Partially update a system option
  data, status_code, headers = api_instance.patch_system_option_with_http_info(option_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <EmptyEnvelope>
rescue OpenapiClient::ApiError => e
  puts "Error when calling OptionsApi->patch_system_option_with_http_info: #{e}"
end
```
