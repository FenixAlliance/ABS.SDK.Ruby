  ...
  class LicenseFeatureCreateDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::LicenseFeatureCreateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::LicenseFeatureCreateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'license_type_id')
        self.license_type_id = attributes[:'license_type_id']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @value.to_s.length > 500
        invalid_properties.push('invalid value for "value", the character length must be smaller than or equal to 500.')
      end
    ...
    def valid?
      ...
      return false if @value.to_s.length > 500
    ...
    def value=(value)
      ...
      if value.to_s.length > 500
        fail ArgumentError, 'invalid value for "value", the character length must be smaller than or equal to 500.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          license_type_id == o.license_type_id
    end
    ...
    def hash
      [id, timestamp, code, key, value, name, description, license_type_id].hash
    end
