...
module OpenapiClient
  class SocialMediaPostUpdateDto
    attr_accessor :title
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SocialMediaPostUpdateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SocialMediaPostUpdateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@title.nil? && @title.to_s.length > 99
        invalid_properties.push('invalid value for "title", the character length must be smaller than or equal to 99.')
      end
    ...
    def title=(title)
      if !title.nil? && title.to_s.length > 99
        fail ArgumentError, 'invalid value for "title", the character length must be smaller than or equal to 99.'
      end
    ...
    def social_post_bucket_id=(social_post_bucket_id)
      if !social_post_bucket_id.nil? && social_post_bucket_id.to_s.length > 36
        fail ArgumentError, 'invalid value for "social_post_bucket_id", the character length must be smaller than or equal to 36.'
      end

      if !social_post_bucket_id.nil? && social_post_bucket_id.to_s.length < 0
        fail ArgumentError, 'invalid value for "social_post_bucket_id", the character length must be great than or equal to 0.'
      end
    ...
    def hash
      [title, content, featured_image_url, social_post_bucket_id].hash
    end
