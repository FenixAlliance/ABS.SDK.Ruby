...
opts = {
  ...
  item_brand_create_dto: OpenapiClient::ItemBrandCreateDto.new({name: 'name_example'}) # ItemBrandCreateDto | 
}
...
> patch_item_brand_async(tenant_id, item_brand_id, opts)
...
begin
  # Patch an item brand
  api_instance.patch_item_brand_async(tenant_id, item_brand_id, opts)
rescue OpenapiClient::ApiError => e
  puts "Error when calling ItemBrandsApi->patch_item_brand_async: #{e}"
end
```

#### Using the patch_item_brand_async_with_http_info variant

This returns an Array which contains the response data (`nil` in this case), status code and headers.

> <Array(nil, Integer, Hash)> patch_item_brand_async_with_http_info(tenant_id, item_brand_id, opts)

```ruby
begin
  # Patch an item brand
  data, status_code, headers = api_instance.patch_item_brand_async_with_http_info(tenant_id, item_brand_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue OpenapiClient::ApiError => e
  puts "Error when calling ItemBrandsApi->patch_item_brand_async_with_http_info: #{e}"
end
```
