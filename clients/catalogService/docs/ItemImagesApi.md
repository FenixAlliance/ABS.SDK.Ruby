...
opts = {
  ...
  item_image_create_dto: OpenapiClient::ItemImageCreateDto.new({business_id: 'business_id_example', file_name: 'file_name_example'}) # ItemImageCreateDto | 
}
...
> <ItemImageDtoListEnvelope> get_item_images_async(tenant_id, opts)
...
begin
  # Get all item images
  result = api_instance.get_item_images_async(tenant_id, opts)
  p result
rescue OpenapiClient::ApiError => e
...
> <Array(<ItemImageDtoListEnvelope>, Integer, Hash)> get_item_images_async_with_http_info(tenant_id, opts)

```ruby
begin
  # Get all item images
  data, status_code, headers = api_instance.get_item_images_async_with_http_info(tenant_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
