...
> <ItemQuestionDtoListEnvelope> get_item_questions_async(tenant_id, opts)
...
begin
  # Get all item questions
  result = api_instance.get_item_questions_async(tenant_id, opts)
  p result
rescue OpenapiClient::ApiError => e
...
> <Array(<ItemQuestionDtoListEnvelope>, Integer, Hash)> get_item_questions_async_with_http_info(tenant_id, opts)

```ruby
begin
  # Get all item questions
  data, status_code, headers = api_instance.get_item_questions_async_with_http_info(tenant_id, opts)
  p status_code # => 2xx
  p headers # => { ... }
