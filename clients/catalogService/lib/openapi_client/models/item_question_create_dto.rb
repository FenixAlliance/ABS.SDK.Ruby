  ...
  class ItemQuestionCreateDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ItemQuestionCreateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ItemQuestionCreateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'needs_revision')
        self.needs_revision = attributes[:'needs_revision']
      else
        self.needs_revision = nil
      end
    ...
    def list_invalid_properties
      ...
      if @needs_revision.nil?
        invalid_properties.push('invalid value for "needs_revision", needs_revision cannot be nil.')
      end
      ...
      if @question.to_s.length < 0
        invalid_properties.push('invalid value for "question", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @needs_revision.nil?
    ...
    def question=(question)
      ...
      if question.to_s.length < 0
        fail ArgumentError, 'invalid value for "question", the character length must be great than or equal to 0.'
      end
