  ...
  class ItemCategoryCreateDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ItemCategoryCreateDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ItemCategoryCreateDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@parent_item_category_id.nil? && @parent_item_category_id.to_s.length < 36
        invalid_properties.push('invalid value for "parent_item_category_id", the character length must be great than or equal to 36.')
      end
    ...
    def parent_item_category_id=(parent_item_category_id)
      if !parent_item_category_id.nil? && parent_item_category_id.to_s.length > 36
        fail ArgumentError, 'invalid value for "parent_item_category_id", the character length must be smaller than or equal to 36.'
      end

      if !parent_item_category_id.nil? && parent_item_category_id.to_s.length < 36
        fail ArgumentError, 'invalid value for "parent_item_category_id", the character length must be great than or equal to 36.'
      end

      @parent_item_category_id = parent_item_category_id
    end
